...
export function Tile({ index, state, disabled, onClick, currentMultiplier }: TileProps) {
  ...
  if (state === 'safe') {
    ...
    safeItem = (
      <span 
        className={`${isBig ? 'text-6xl drop-shadow-[0_0_35px_rgba(167,139,250,1)] animate-pulse brightness-125' : 'text-5xl drop-shadow-[0_0_20px_rgba(45,212,191,0.8)]'} animate-diamond-pop relative z-20`} 
        role="img" 
        aria-label="diamond"
      >
        💎
  ...
  return (
    ...
    >
      {/* Light Flare Burst (Safe) */}
      {animating && state === 'safe' && (
      ...
      {/* Explosion Effects (Mine) */}
      {animating && state === 'mine' && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
      ...
      {/* Decorative inner glow for safe tiles */}
      {state === 'safe' && (
      ...
      {state === 'mine' && (
        <span className="text-6xl drop-shadow-[0_0_35px_rgba(239,68,68,1)] filter brightness-125 relative z-20" role="img" aria-label="bomb">
          💣
        </span>
      )}
    </button>

...
import { generateServerSeed, hashServerSeed, generateMinePositions, } from './provably-fair.service';
 ...
 */
async function settleWithLobby(lobbySessionId, betAmount, payout, betId) {
    if (!lobbySessionId)
        return undefined;
    try {
...
export async function startGame(betAmount, mineCount, rtp, lobbyToken, lobbySessionId) {
...
export async function pickTile(sessionId, tileIndex) {
    ...
    if (isFullClear) {
        ...
        const newBalance = await settleWithLobby(session.lobbySessionId, 0, payout, session.betId);
        return {
            result: 'safe',
            newMultiplier,
...
export async function cashout(sessionId) {
    const session = getSession(sessionId);
    if (!session) {
        throw new GameError(404, 'Session not found');
    }
